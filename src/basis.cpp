...
void BasisSet::projectOMOs(const BasisSet & oldbas, const arma::cx_mat & oldOMOs, arma::cx_mat & OMOs, size_t nocc) const {
  ...
  // Calculate overlap of old OMOs in new basis
  arma::cx_mat SO(arma::trans(oldOMOs.cols(0,nocc-1))*S*oldOMOs.cols(0,nocc-1));

  // Run eigendecomposition
  arma::vec SOval;
  arma::cx_mat SOvec;
  bool ok=arma::eig_sym(SOval,SOvec,SO);
  if(!ok)
    throw std::runtime_error("Failed to decompose OMO overlap matrix!\n");

  // Reciprocal eigenvalues
  arma::vec SOinval(SOval);
  for(size_t i=0;i<SOinval.n_elem;i++)
    if(SOval(i)>=LINTHRES)
      SOinval(i)=1.0/SOval(i);
    else {
      std::ostringstream oss;
      oss << "Too small OMO overlap eigenvalue " << SOval(i) << "!\n";
      throw std::runtime_error(oss.str());
    }
  
  // Symmetric orthogonalizing matrix
  arma::cx_mat O(SOvec*arma::diagmat(SOinval)*arma::trans(SOvec));

  // Orthonormalized orbitals
  OMOs.zeros(S.n_rows,Sinvh.n_cols);
  OMOs.cols(0,nocc-1)=oldOMOs.cols(0,nocc-1)*O;

  // Dig out OMOs from Sinvh
  arma::cx_mat X(arma::trans(OMOs.cols(0,nocc-1))*S*Sinvh);
  arma::vec s;
  arma::cx_mat U;
  arma::cx_mat V;
  ok=arma::svd(U,s,V,X);
  ...
  // Virtuals are
  OMOs.cols(nocc,OMOs.n_cols-1)=Sinvh*V.cols(nocc,V.n_cols-1);

  // Failsafe
  try {
    // Check orthogonality of orbitals
    check_orth(OMOs,S,false);
  } catch(std::runtime_error & err) {
    std::ostringstream oss;
