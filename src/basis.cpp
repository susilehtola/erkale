...
void BasisSet::projectMOs(const BasisSet & oldbas, const arma::colvec & oldE, const arma::mat & oldMOs, arma::colvec & E, arma::mat & MOs) const {
  ...
  try {
    ...
    printf("Nmo = %i, Nind = %i, Nbf = %i.\n",(int) Nmo,(int) Nind, (int) Nbf);

    arma::mat ovl=arma::trans(MOs)*S11*MOs;
    ovl.print("Orbital overlap");
   
    std::ostringstream oss;
    oss << "Orbitals generated by projectMOs are not orthonormal: " << err.what();
    throw std::runtime_error(oss.str());
  }
}
