...
class SCF {
 ...
 /// Overlap matrix
  ...
  void PZSIC_RDFT(rscf_t & sol, const std::vector<double> & occs, dft_t dft, const DFTGrid & grid, size_t iter, bool canonical=false, bool localize=true);
  /// Perform Perdew-Zunger self-interaction correction
  void PZSIC_UDFT(uscf_t & sol, const std::vector<double> & occa, const std::vector<double> & occb, dft_t dft, const DFTGrid & grid, size_t iter, bool canonical=false, bool localize=true);
