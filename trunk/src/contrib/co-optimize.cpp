...
void compute_density_gaussian(int Z, bool dimer, const BasisSetLibrary & baslib, std::string method, BasisSet & basis, arma::mat & P, int & Nel, double & E) {
  ...
  */
  ...
  //  fprintf(out,"#P GFINPUT Density=Current Guess=Core SCF=(DIIS,IntRep,NoIncFock,DSymm,NoVarAcc)\n");
  fprintf(out,"#P GFINPUT Density=Current Guess=Core SCF=(XQC,IntRep,NoIncFock,DSymm,NoVarAcc) Integral=(Acc2E=12)\n");
  ...
  if(g09val) {
    printf("Error running Gaussian '09. Trying again with CDIIS.\n");

    /* Try with conventional DIIS */
    ...
    fprintf(out,"%%Mem=%s\n",memory.c_str());
    fprintf(out,"%%NProcShared=%i\n",nthreads);
    // DIIS for g1-g4, CDIIS for g5
    fprintf(out,"#P GFINPUT Density=Current Guess=Core SCF=(CDIIS,IntRep,NoIncFock,DSymm,NoVarAcc) Integral=(Acc2E=12)\n");
  ...
  if(g09val) {
    printf("Error running Gaussian '09. Trying again with DIIS and the default guess.\n");
    ...
    fprintf(out,"%%NProcShared=%i\n",nthreads);
    // DIIS for g1-g4, CDIIS for g5
    fprintf(out,"#P GFINPUT Density=Current SCF=(CDIIS,IntRep,NoIncFock,DSymm,NoVarAcc) Integral=(Acc2E=12)\n");
  ...
  if(g09val) {
    printf("Error running Gaussian '09. Relaxing constraints.\n");
  ...
  if(g09val) {
    printf("Error running Gaussian '09. Keeping constraints but redo SCF only once.\n");
    ...
    //  fprintf(out,"#SP, %s/Gen\n",method.c_str());
    if(polmethod == RHF)
      fprintf(out,"#Stable=(1Opt,RRHF,XQC), %s/Gen\n",method.c_str());
    else
      fprintf(out,"#Stable=(1Opt,RUHF,XQC), %s/Gen\n",method.c_str());
  ...
  if(g09val) {
    printf("Error running Gaussian '09. Disabling stability analysis.\n");
